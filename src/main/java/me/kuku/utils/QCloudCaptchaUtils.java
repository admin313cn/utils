...
public class QCloudCaptchaUtils {
	...
	private static String getCaptchaPictureUrl(String appId, String sig, String sess, String sid, String webSig, String vSig, int index){
		int subSid = 3;
		return String.format("https://captcha.guard.qcloud.com/cap_union_new_getcapbysig?asig=%s&aid=%s&captype=&protocol=https&clientype=1&disturblevel=&apptype=&noheader=0&color=&showtype=popup&fb=1&theme=&lang=2052&ua=%s&sess=%s&fwidth=0&sid=%s&subsid=%s&uid=&cap_cd=&rnd=%s&forcestyle=undefined&wxLang=&TCapIframeLoadTime=19&prehandleLoadTime=129&createIframeStart=%s&rand=0.0%s&websig=%s&vsig=%s&img_index=%s",
				sig, appId, ua, sess, sid, subSid, MyUtils.randomNum(6), System.currentTimeMillis(), MyUtils.randomNum(15), webSig, vSig, index);
	}
	...
	private static Map<String, String> getCaptcha(String appId, String sig) throws IOException {
		JSONObject jsonObject = OkHttpUtils.getJsonp(String.format("https://captcha.guard.qcloud.com/cap_union_prehandle?asig=%s&aid=%s&captype=&protocol=https&clientype=1&disturblevel=&apptype=&noheader=0&color=&showtype=popup&fb=1&theme=&lang=2052&ua=%s&cap_cd=&uid=&callback=_aq_45812&sess=&subsid=1",
				sig, appId, ua));
		String url = String.format("https://captcha.guard.qcloud.com/cap_union_new_show?asig=%s&aid=%s&captype=&protocol=https&clientype=1&disturblevel=&apptype=&noheader=0&color=&showtype=popup&fb=1&theme=&lang=2052&ua=%s&sess=%s&fwidth=0&sid=%s&subsid=2&uid=&cap_cd=&rnd=%s&forcestyle=undefined&wxLang=&TCapIframeLoadTime=19&prehandleLoadTime=129&createIframeStart=%s",
				sig, appId, ua, jsonObject.getString("sess"), jsonObject.getString("sid"), MyUtils.randomNum(6), System.currentTimeMillis());
		...
		String imageAUrl = getCaptchaPictureUrl(appId, sig, sess, jsonObject.getString("sid"), webSig, vSig, 1);
		String imageBUrl = getCaptchaPictureUrl(appId, sig, sess, jsonObject.getString("sid"), webSig, vSig, 0);
	...
	private static Result<TencentCaptcha> identifyCaptcha(String appId, Map<String, String> map) throws IOException {
		...
		paramsMap.put("rnd", String.valueOf(MyUtils.randomInt(100000, 999999)));
		paramsMap.put("forcestyle", "undefined");
		paramsMap.put("wxLang", "");
		paramsMap.put("TCapIframeLoadTime", "19");
		paramsMap.put("prehandleLoadTime", "129");
